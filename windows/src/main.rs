...
fn main() {
    ...
    if args[1] == "--help" || args[1] == "-h" {
        ...
        println!(
            "       By setting options, you can choose whether to rewrite or continue writing\n"
        );
        println!("       writeoptions:");
        println!("       -r:");
        println!(
            "              Rewrites the contents of the file (existing contents well be lost)\n"
        );
                ...
                if args[3] == "-r" && args.len() == 5 {
                    ...
                    if path.exists() {
                        ...
                        if trimstr == "y" || trimstr == "yes" {
                            let mut file =
                                File::create(path).expect("Error: failed to create file.");
                            let contents = &args[4];
                            file.write_all(contents.as_bytes())
                                .expect("Error: failed to write file");
                            println!("Successfully wrote to the file.");
                    ...
                    let path = &args[1];
                    let mut file = OpenOptions::new().append(true).open(path).expect("msg");
                    let contents = &args[4];
